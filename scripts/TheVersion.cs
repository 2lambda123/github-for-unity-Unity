...
public struct TheVersion {
  private const string versionRegex =
      @"^(?<major>\d+)(\.?(?<minor>\d+))?(\.?(?<patch>\d+))?(\.?(?<build>\d+))?(\.?(?<special>.+))?";
  ...
  public string Version {
    get { return version ?? (version = string.Empty); }
    set { version = value; }
  }
  ...
  public TheVersion BumpPatch() {
    return ResetFromBuild(
        string.Format("{0}.{1}.{2}", Major, Minor, Patch + 1));
  }

  public TheVersion BumpBuild() {
    return ResetFromSpecial(
        string.Format("{0}.{1}.{2}.{3}", Major, Minor, Patch, Build + 1));
  }
  ...
  public TheVersion SetBuild(int newValue) {
    return ResetFromSpecial(
        string.Format("{0}.{1}.{2}.{3}", Major, Minor, Patch, newValue));
  }
