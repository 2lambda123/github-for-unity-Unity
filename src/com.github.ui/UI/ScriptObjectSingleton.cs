  ...
  public string FilePath {
    get {
      ...
      if (location == Location.PreferencesFolder)
        filePath =
            InternalEditorUtility.unityPreferencesFolder + "/" + relativePath;
      else if (location == Location.UserFolder)
        filePath = EntryPoint.ApplicationManager.Environment.UserCachePath
                       .Combine(relativePath)
                       .ToString(SlashMode.Forward);
      else if (location == Location.LibraryFolder)
        filePath =
            EntryPoint.ApplicationManager.Environment.UnityProjectPath.Combine(
                "Library", "gfu", relativePath);
...
class ScriptObjectSingleton<T> : ScriptableObject
  ...
  protected virtual void Save(bool saveAsText) {
    ...
    if (locationFilePath != null) {
      ...
      InternalEditorUtility.SaveToSerializedFileAndForget(
          new[] { instance }, locationFilePath, saveAsText);
    }
  }
  ...
  private static NPath? GetFilePath() {
    ...
    // LogHelper.Instance.Debug("FilePath {0}", attr != null ? attr.filepath :
    // null);
