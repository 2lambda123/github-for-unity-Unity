...
class GUIPackage {
  ...
  public Package Package {
    get {
      if (package == null) {
        package = new Package { Version = TheVersion.Parse(version), Url = url,
...
class UpdateCheckWindow : BaseWindow {
  ...
  public static void CheckForUpdates(IApplicationManager manager) {
    var download =
        new DownloadTask(manager.TaskManager.Token,
                         manager.Environment.FileSystem, UpdateFeedUrl,
                         manager.Environment.UserCachePath)
            .Catch(ex => {
              LogHelper.Warning(@"Error downloading update check:{0} ""{1}""",
                                UpdateFeedUrl, ex.GetExceptionMessageShort());
              return true;
            });
    download.OnEnd += (thisTask, result, success, exception) => {
      if (success) {
        try {
          Package package = result.ReadAllText().FromJson<Package>(
              lowerCase: true, onlyPublic: false);
          TheVersion current = TheVersion.Parse(ApplicationInfo.Version);
          TheVersion newVersion = package.Version;

          var versionToSkip =
              manager.UserSettings.Get<TheVersion>(Constants.SkipVersionKey);
          ...
          if (newVersion <= current) {
            LogHelper.Trace("Skipping GitHub for Unity update v" + newVersion +
                            ", we already have it");
            return;
          }
  ...
  private static void NotifyOfNewUpdate(IApplicationManager manager,
                                        TheVersion currentVersion,
                                        Package package) {
    var window = GetWindowWithRect<UpdateCheckWindow>(
        new Rect(100, 100, 580, 400), true, windowTitle);
    window.Initialize(manager, currentVersion, package);
    window.Show();
  }

  private const string windowTitle = "GitHub for Unity Update Check";
  private const string newUpdateMessage =
      "There is a new version of GitHub for Unity available.\n\nCurrent version is {0}\nNew version is {1}";
  ...
  [SerializeField]
  private bool hasReleaseNotesUrl;
  [SerializeField]
  private bool hasMessage;

  private void Initialize(IApplicationManager manager, TheVersion current,
                          Package newPackage) {
  ...
  public override void OnUI() {
    ...
    GUILayout.Space(10);
    GUI.Box(new Rect(13, 8, guiLogo.image.width, guiLogo.image.height), guiLogo,
            GUIStyle.none);
    ...
    EditorGUILayout.LabelField(guiNewUpdate, "WordWrappedLabel",
                               GUILayout.Width(300));
    ...
    if (hasMessage || hasReleaseNotes) {
      ...
      scrollPos = EditorGUILayout.BeginScrollView(
          scrollPos, GUILayout.Width(405), GUILayout.Height(200));
      if (hasMessage) {
    ...
    if (GUILayout.Button(guiSkipThisVersion, GUILayout.Width(200))) {
      ...
      settings.Set<TheVersion>(Constants.SkipVersionKey,
  ...
  private void LoadContents() {
    ...
    guiNewUpdate = new GUIContent(String.Format(
        newUpdateMessage, currentVersion, package.Package.Version.ToString()));
