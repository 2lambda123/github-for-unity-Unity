...
class AuthenticationService {
  ...
  public AuthenticationService(UriString host, IKeychain keychain,
    ...
    client =
        new ApiClient(keychain, processManager, taskManager, environment, host);
  }

  public HostAddress HostAddress {
    get { return client.HostAddress; }
  }

  public void Login(string username, string password,
                    Action<string> twofaRequired,
                    Action<bool, string> authResult) {
  ...
  public void GetServerMeta(Action<GitHubHostMeta> serverMeta,
                            Action<string> error) {
    loginResultData = null;
    client.GetEnterpriseServerMeta(data => { serverMeta(data); },
  ...
  public Uri GetLoginUrl(string state) {
    ...
    query.Append(
        Uri.EscapeDataString(OAuthCallbackManager.CallbackUrl.ToString()));
    query.Append("&scope=");
