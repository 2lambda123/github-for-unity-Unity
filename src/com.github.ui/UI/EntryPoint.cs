...
public class EntryPoint : ScriptableObject {
  ...
  // we do this so we're guaranteed to run on the main thread, not the loader
  // thread
  private static void Initialize() {
    EditorApplication.update -= Initialize;

    // this will initialize ApplicationManager and Environment if they haven't
    // yet
    var logPath = ApplicationManager.Environment.LogPath;

    if (ApplicationCache.Instance.FirstRun) {
      var oldLogPath = logPath.Parent.Combine(
          logPath.FileNameWithoutExtension + "-old" + logPath.ExtensionWithDot);
      try {
      ...
      Debug.LogFormat(
          "Initialized Git for Unity version {0}{1} ({2}) Log file: {2}",
          ApplicationInfo.Version, ApplicationManager.Environment.NewLine,
          logPath);
    }
    ...
    LogHelper.Info("Initializing GitForUnity:'v{0}' Unity:'v{1}'",
                   ApplicationInfo.Version,
                   ApplicationManager.Environment.UnityVersion);
  ...
  public static IApplicationManager ApplicationManager {
    get {
      if (appManager == null) {
        appManager =
            new ApplicationManager(new MainThreadSynchronizationContext(),
                                   EnvironmentCache.Instance.Environment);
      }
      return appManager;
    }
  }
