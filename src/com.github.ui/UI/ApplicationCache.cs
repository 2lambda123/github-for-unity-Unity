    ...
    get {
      ...
      if (!firstRunAtValue.HasValue) {
        ...
        if (!DateTimeOffset.TryParseExact(
                firstRunAtString.ToEmptyIfNull(), Constants.Iso8601Formats,
...
sealed class EnvironmentCache : ScriptObjectSingleton<EnvironmentCache> {
  ...
  [SerializeField]
  private string extensionInstallPath;
  [SerializeField]
  private string repositoryPath;
  [SerializeField]
  private string unityApplication;
  [SerializeField]
  private string unityApplicationContents;
  [SerializeField]
  private string unityAssetsPath;
  [SerializeField]
  ...
  private NPath DetermineInstallationPath() {
    ...
    var scriptPath = Application.dataPath.ToNPath().Parent.Combine(
        AssetDatabase.GetAssetPath(script).ToNPath());
    DestroyImmediate(shim);
    var potentialBasePath = scriptPath.Parent;
    try {
      if (!potentialBasePath.DirectoryExists()) {
        // we might be in a package fake folder, look for the real one in the
        // manifest
        var packageRoot = potentialBasePath.RecursiveParents.FirstOrDefault(
            p => p.FileName == "Packages" &&
                 p.Combine("manifest.json").FileExists());
        if (packageRoot.IsInitialized) {
          var packageList = packageRoot.Combine("manifest.json")
                                .ReadAllText()
                                .FromJson<ManifestJson>(true);
          if (packageList.dependencies.TryGetValue("com.github.ui",
                                                   out string relpath)) {
            potentialBasePath =
                packageRoot.Combine(relpath, potentialBasePath.FileName)
                    .MakeAbsolute();
          }
        }
      }
  ...
  public IEnvironment Environment {
    get {
      if (environment == null) {
        ...
        cacheContainer.SetCacheInitializer(CacheType.Branches,
                                           () => BranchesCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.GitAheadBehind,
                                           () => GitAheadBehindCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.GitLocks,
                                           () => GitLocksCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.GitLog,
                                           () => GitLogCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.GitFileLog,
                                           () => GitFileLogCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.GitStatus,
                                           () => GitStatusCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.GitUser,
                                           () => GitUserCache.Instance);
        cacheContainer.SetCacheInitializer(CacheType.RepositoryInfo,
                                           () => RepositoryInfoCache.Instance);
        ...
        environment.Initialize(unityVersion, extensionInstallPath.ToNPath(),
...
abstract class ManagedCacheBase<T> : ScriptObjectSingleton<T>
    where T : ScriptableObject, IManagedCache {
  [SerializeField]
  private CacheType cacheType;
  [SerializeField]
  private string lastUpdatedAtString =
      DateTimeOffset.MinValue.ToString(Constants.Iso8601Format);
  [SerializeField]
  private string initializedAtString =
      DateTimeOffset.MinValue.ToString(Constants.Iso8601Format);
  [NonSerialized]
  ...
  public bool ValidateData() {
    ...
    if (needsInvalidation && !isInvalidating) {
      Logger.Trace("needsInvalidation isInitialized:{0} timedOut:{1}",
                   isInitialized, timedOut);
      Invalidate();
    }
  ...
  public void ResetInvalidation() { isInvalidating = false; }

  protected void SaveData(DateTimeOffset now, bool isChanged) {
    var isInitialized = IsInitialized;
    isChanged = isChanged || !isInitialized;

    InitializedAt = !isInitialized || InitializedAt == DateTimeOffset.MinValue
                        ? now
                        : InitializedAt;
    LastUpdatedAt = isChanged || LastUpdatedAt == DateTimeOffset.MinValue
                        ? now
                        : LastUpdatedAt;
  ...
  public DateTimeOffset LastUpdatedAt {
    get {
      if (!lastUpdatedAtValue.HasValue) {
        ...
        if (DateTimeOffset.TryParseExact(LastUpdatedAtString.ToEmptyIfNull(),
  ...
  public DateTimeOffset InitializedAt {
    get {
      if (!initializedAtValue.HasValue) {
        DateTimeOffset result;
        if (DateTimeOffset.TryParseExact(InitializedAtString.ToEmptyIfNull(),
                                         Constants.Iso8601Formats,
                                         CultureInfo.InvariantCulture,
                                         DateTimeStyles.None, out result)) {
...
[Serializable]
class LocalConfigBranchDictionary
    : SerializableDictionary<string, ConfigBranch>,
      ILocalConfigBranchDictionary {
  public LocalConfigBranchDictionary() {}

  public LocalConfigBranchDictionary(
      IDictionary<string, ConfigBranch> dictionary)
      : base() {
...
class RemoteConfigBranchDictionary
    : Dictionary<string, Dictionary<string, ConfigBranch>>,
      ISerializationCallbackReceiver,
      IRemoteConfigBranchDictionary {
  [SerializeField]
  private string[] keys = new string[0];
  [SerializeField]
  private StringArrayContainer[] subKeys = new StringArrayContainer[0];
  [SerializeField]
  private ConfigBranchArrayContainer[] subKeyValues =
      new ConfigBranchArrayContainer[0];

  public RemoteConfigBranchDictionary() {}

  public RemoteConfigBranchDictionary(
      Dictionary<string, Dictionary<string, ConfigBranch>> dictionary) {
    foreach (var pair in dictionary) {
      Add(pair.Key, pair.Value.ToDictionary(valuePair => valuePair.Key,
  ...
  public void OnBeforeSerialize() {
    ...
    foreach (var pair in this) {
      ...
      subKeysList.Add(
          new StringArrayContainer { Values = serializeSubKeys.ToArray() });
      subKeysValuesList.Add(new ConfigBranchArrayContainer {
        Values = serializeSubKeyValues.ToArray()
      });
    }
  ...
  public void OnAfterDeserialize() {
    ...
    if (keys.Length != subKeys.Length ||
        subKeys.Length != subKeyValues.Length) {
    ...
    for (var remoteIndex = 0; remoteIndex < keys.Length; remoteIndex++) {
      ...
      for (var branchIndex = 0; branchIndex < subKeyContainer.Values.Length;
           branchIndex++) {
...
[Serializable]
class ConfigRemoteDictionary : SerializableDictionary<string, ConfigRemote>,
                               IConfigRemoteDictionary {
...
sealed class RepositoryInfoCache : ManagedCacheBase<RepositoryInfoCache>,
                                   IRepositoryInfoCache {
  [SerializeField]
  ...
  public void UpdateData(IRepositoryInfoCacheData data) {
    ...
    if (forcedInvalidation ||
        !Nullable.Equals(currentGitRemote, data.CurrentGitRemote)) {
      currentGitRemote = data.CurrentGitRemote ?? GitRemote.Default;
      isUpdated = true;
    }

    if (forcedInvalidation ||
        !Nullable.Equals(currentGitBranch, data.CurrentGitBranch)) {
      currentGitBranch = data.CurrentGitBranch ?? GitBranch.Default;
      isUpdated = true;
    }

    if (forcedInvalidation ||
        !Nullable.Equals(currentConfigRemote, data.CurrentConfigRemote)) {
      currentConfigRemote = data.CurrentConfigRemote ?? ConfigRemote.Default;
      isUpdated = true;
    }

    if (forcedInvalidation ||
        !Nullable.Equals(currentConfigBranch, data.CurrentConfigBranch)) {
  ...
  public GitRemote? CurrentGitRemote {
    get {
      ValidateData();
      return currentGitRemote.Equals(GitRemote.Default) ? (GitRemote?)null
                                                        : currentGitRemote;
    }
  }
  ...
  public GitBranch? CurrentGitBranch {
    get {
      ValidateData();
      return currentGitBranch.Equals(GitBranch.Default) ? (GitBranch?)null
                                                        : currentGitBranch;
    }
  }
  ...
  public ConfigRemote? CurrentConfigRemote {
    get {
      ValidateData();
      return currentConfigRemote.Equals(ConfigRemote.Default)
                 ? (ConfigRemote?)null
                 : currentConfigRemote;
    }
  }
  ...
  public ConfigBranch? CurrentConfigBranch {
    get {
      ValidateData();
      return currentConfigBranch.Equals(ConfigBranch.Default)
                 ? (ConfigBranch?)null
                 : currentConfigBranch;
    }
  }
...
sealed class BranchesCache : ManagedCacheBase<BranchesCache>, IBranchCache {
  ...
  [SerializeField]
  private LocalConfigBranchDictionary localConfigBranches =
      new LocalConfigBranchDictionary();
  [SerializeField]
  private RemoteConfigBranchDictionary remoteConfigBranches =
      new RemoteConfigBranchDictionary();
  [SerializeField]
  ...
  public void SetRemotes(
      Dictionary<string, ConfigRemote> remoteConfigs,
      Dictionary<string, Dictionary<string, ConfigBranch>> configBranches,
      GitRemote[] gitRemotes, GitBranch[] gitBranches) {
    var now = DateTimeOffset.Now;
  ...
  public void SetLocals(Dictionary<string, ConfigBranch> configBranches,
...
sealed class GitFileLogCache : ManagedCacheBase<GitFileLogCache>,
                               IGitFileLogCache {
  [SerializeField]
  private GitFileLog fileLog = GitFileLog.Default;
...
[Location("cache/gittrackingstatus.yaml",
          LocationAttribute.Location.LibraryFolder)]
sealed class GitAheadBehindCache : ManagedCacheBase<GitAheadBehindCache>,
                                   IGitAheadBehindCache {
  [SerializeField]
  private int ahead;
  [SerializeField]
  private int behind;
...
[Location("cache/gitstatusentries.yaml",
          LocationAttribute.Location.LibraryFolder)]
sealed class GitStatusCache : ManagedCacheBase<GitStatusCache>,
                              IGitStatusCache {
  [SerializeField]
