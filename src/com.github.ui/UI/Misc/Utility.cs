...
class Utility : ScriptableObject {
  private static Dictionary<string, Texture2D> iconCache =
      new Dictionary<string, Texture2D>();

  public static bool IsDarkTheme {
    get {
      var defaultTextColor = Styles.Label.normal.textColor;
      return defaultTextColor.r > 0.5f && defaultTextColor.g > 0.5f &&
             defaultTextColor.b > 0.5f;
    }
  }
  ...
  public static Texture2D GetIcon(string filename, string filename2x = "",
    ...
    if (EditorGUIUtility.pixelsPerPoint > 1f &&
        !string.IsNullOrEmpty(filename2x)) {
      filename = filename2x;
    }
    ...
    using (var stream = AssemblyResources.ToStream(
               ResourceType.Icon, filename,
               EntryPoint.ApplicationManager.Environment)) {
...
static class StreamExtensions {
  ...
  static StreamExtensions() {
    ...
    loadImage = typeof(Texture2D).GetMethods().FirstOrDefault(
        x => x.Name == "LoadImage" && x.GetParameters().Length == 1);
    if (loadImage != null) {
      ...
      // 2017.1
      var t = typeof(Texture2D).Assembly.GetType("UnityEngine.ImageConversion",
                                                 false, false);
      if (t == null) {
        // 2017.2 and above
        t = Assembly.Load("UnityEngine.ImageConversionModule")
                .GetType("UnityEngine.ImageConversion", false, false);
      }

      if (t != null) {
        // looking for ImageConversion.LoadImage(this Texture2D tex, byte[]
        // data)
        loadImage = t.GetMethods().FirstOrDefault(
            x => x.Name == "LoadImage" && x.GetParameters().Length == 2);
        invokeLoadImage = (tex, ms) => {
