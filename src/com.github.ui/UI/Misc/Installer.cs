...
class Installer : ScriptableObject {
  ...
  public static void Initialize(string selfPath) {
    // Perform move
    if (EditorUtility.DisplayDialog(QueryTitle, QueryMessage, QueryOK,
                                    QueryCancel)) {
      MoveFrom(Application.dataPath +
               selfPath.Substring("Assets".Length,
                                  selfPath.LastIndexOf('/') - "Assets".Length));
    }
  }
  ...
  private static void MoveFrom(string path) {
    try {
      ...
      // TODO: Create the necessary structure and perform the actual move of
      // files into it from the given install path
    } catch (Exception e) {
      Failure(e.ToString());
    }
  }
  ...
  private static void Failure(string error) {
    EditorUtility.DisplayDialog(ErrorTitle, String.Format(ErrorMessage, error),
                                ErrorOK);
  }
}
}
