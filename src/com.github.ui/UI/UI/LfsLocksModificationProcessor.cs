...
class LfsLocksModificationProcessor : UnityEditor.AssetModificationProcessor {
  private static ILogging Logger =
      LogHelper.GetLogger<LfsLocksModificationProcessor>();
  private static IRepository repository;
  private static IPlatform platform;
  private static IEnvironment environment;

  private static Dictionary<NPath, GitLock> locks =
      new Dictionary<NPath, GitLock>();
  ...
  public static string[] OnWillSaveAssets(string[] paths) { return paths; }

  public static AssetMoveResult OnWillMoveAsset(string oldPath,
                                                string newPath) {
    return IsLockedBySomeoneElse(oldPath) || IsLockedBySomeoneElse(newPath)
  ...
  public static AssetDeleteResult OnWillDeleteAsset(string assetPath,
    ...
    return IsLockedBySomeoneElse(assetPath) ? AssetDeleteResult.FailedDelete
  ...
  public static bool IsOpenForEdit(string assetPath, out string message) {
    ...
    canEdit &= !IsLockedBySomeoneElse(lck);
    message = !canEdit ? string.Format("File is locked for editing by {0}",
                                       lck.Value.Owner.Name)
                       : null;
    return canEdit;
  }
  ...
  private static void UserMayHaveChanged() {
    loggedInUser =
        platform.Keychain.Connections.Select(x => x.Username).FirstOrDefault();
  }
  ...
  private static void InspectorHeaderFinished(Editor editor) {
    string message = "";
    if (!IsOpenForEdit(AssetDatabase.GetAssetPath(editor.target),
                       out message)) {
      ...
      GUILayout.BeginVertical();
      {
        ...
        GUILayout.BeginHorizontal();
        {
          ...
          {
            GUILayout.Label(Utility.GetIcon("big-logo.png", "big-logo@2x.png",
                                            Utility.IsDarkTheme),
