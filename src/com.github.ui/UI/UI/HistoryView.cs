...
public class HistoryControl {
  ...
  [NonSerialized]
  private Action<GitLogEntry> rightClickNextRender;
  [NonSerialized]
  private GitLogEntry rightClickNextRenderEntry;
  [NonSerialized]
  ...
  public GitLogEntry SelectedGitLogEntry {
    get {
      return SelectedIndex < 0 ? GitLogEntry.Default : entries[SelectedIndex];
    }
  }

  public bool Render(Rect containingRect,
                     Action<GitLogEntry> singleClick = null,
                     Action<GitLogEntry> doubleClick = null,
                     Action<GitLogEntry> rightClick = null) {
    ...
    scroll = GUILayout.BeginScrollView(scroll);
    {
      ...
      var rect =
          new Rect(containingRect.x, containingRect.y, containingRect.width, 0);

      for (var index = 0; index < entries.Count; index++) {
        var entry = entries[index];

        var entryRect =
            new Rect(rect.x, rect.y, rect.width, Styles.HistoryEntryHeight);

        var shouldRenderEntry =
            !(entryRect.y > endDisplay || entryRect.yMax < startDisplay);
        if (shouldRenderEntry && Event.current.type == EventType.Repaint) {
          RenderEntry(entryRect, entry, index);
        }

        var entryRequiresRepaint = HandleInput(
            entryRect, entry, index, singleClick, doubleClick, rightClick);
  ...
  private void RenderEntry(Rect entryRect, GitLogEntry entry, int index) {
    ...
    var summaryRect = new Rect(
        entryRect.x, entryRect.y + Styles.BaseSpacing / 2, entryRect.width,
        Styles.HistorySummaryHeight + Styles.BaseSpacing);
    var timestampRect = new Rect(entryRect.x,
                                 entryRect.yMax - Styles.HistoryDetailsHeight -
                                     Styles.BaseSpacing / 2,
                                 entryRect.width, Styles.HistoryDetailsHeight);

    var hasKeyboardFocus = GUIUtility.keyboardControl == controlId;

    Styles.Label.Draw(entryRect, GUIContent.none, false, false, isSelected,
                      hasKeyboardFocus);
    Styles.HistoryEntrySummaryStyle.Draw(summaryRect, entry.Summary, false,
                                         false, isSelected, hasKeyboardFocus);

    var historyEntryDetail = string.Format(
        HistoryEntryDetailFormat, entry.PrettyTimeString, entry.AuthorName);
    Styles.HistoryEntryDetailsStyle.Draw(timestampRect, historyEntryDetail,
                                         false, false, isSelected,
                                         hasKeyboardFocus);

    if (!string.IsNullOrEmpty(entry.MergeA)) {
      const float MergeIndicatorWidth = 10.28f;
      const float MergeIndicatorHeight = 12f;
      var mergeIndicatorRect =
          new Rect(entryRect.x + 7, summaryRect.y, MergeIndicatorWidth,
                   MergeIndicatorHeight);

      GUI.DrawTexture(mergeIndicatorRect, Styles.MergeIcon);

      DrawTimelineRectAroundIconRect(entryRect, mergeIndicatorRect);

      summaryRect.Set(mergeIndicatorRect.xMax, summaryRect.y,
                      summaryRect.width - MergeIndicatorWidth,
                      summaryRect.height);
    } else {
      if (isLocalCommit) {
        const float LocalIndicatorSize = 6f;
        var localIndicatorRect =
            new Rect(entryRect.x + (Styles.BaseSpacing - 2), summaryRect.y + 5,
                     LocalIndicatorSize, LocalIndicatorSize);

        DrawTimelineRectAroundIconRect(entryRect, localIndicatorRect);

        GUI.DrawTexture(localIndicatorRect, Styles.LocalCommitIcon);

        summaryRect.Set(localIndicatorRect.xMax, summaryRect.y,
                        summaryRect.width - LocalIndicatorSize,
                        summaryRect.height);
      } else {
        const float NormalIndicatorWidth = 6f;
        const float NormalIndicatorHeight = 6f;

        var normalIndicatorRect =
            new Rect(entryRect.x + (Styles.BaseSpacing - 2), summaryRect.y + 5,
  ...
  private bool HandleInput(Rect rect, GitLogEntry entry, int index,
                           Action<GitLogEntry> singleClick = null,
                           Action<GitLogEntry> doubleClick = null,
                           Action<GitLogEntry> rightClick = null) {
    var requiresRepaint = false;
    ...
    if (GUIUtility.keyboardControl == controlId && index == SelectedIndex &&
      ...
      var directionY = Event.current.keyCode == KeyCode.UpArrow     ? -1
                       : Event.current.keyCode == KeyCode.DownArrow ? 1
                                                                    : 0;
      if (directionY != 0) {
        Event.current.Use();
  ...
  private void DrawTimelineRectAroundIconRect(Rect parentRect, Rect iconRect) {
    ...
    Rect topTimelineRect = new Rect(parentRect.x + Styles.BaseSpacing,
                                    parentRect.y, 2, topTimelineRectHeight);
    ...
    Rect bottomTimelineRect =
        new Rect(parentRect.x + Styles.BaseSpacing,
                 parentRect.yMax - bottomTimelineRectHeight, 2,
...
abstract class HistoryBase : Subview {
  ...
  protected void BuildHistoryControl(int loadAhead,
                                     List<GitLogEntry> gitLogEntries) {
  ...
  protected void BuildTreeChanges() {
    TreeChanges.PathSeparator =
        Environment.FileSystem.DirectorySeparatorChar.ToString();
    TreeChanges.Load(SelectedEntry.changes.Select(
        entry => new GitStatusEntryTreeData(entry)));
    Redraw();
  }

  protected void RevertCommit() {
    var dialogTitle = "Revert commit";
    var dialogBody = string.Format(
        @"Are you sure you want to revert the following commit:""{0}""",
        SelectedEntry.Summary);

    if (EditorUtility.DisplayDialog(dialogTitle, dialogBody, "Revert",
                                    "Cancel")) {
      Repository.Revert(SelectedEntry.CommitID)
          .FinallyInUI((success, e) => {
            if (!success) {
              EditorUtility.DisplayDialog(
                  dialogTitle, "Error reverting commit: " + e.Message,
                  Localization.Cancel);
            }
  ...
  protected void
  DoHistoryGui(Rect rect, Action<GitLogEntry> historyControlRightClick = null,
               Action<ChangesTreeNode> changesTreeRightClick = null) {
    if (HistoryControl != null) {
      var historyControlRect =
          new Rect(0f, 0f, Position.width, Position.height - rect.height);
    ...
    if (!SelectedEntry.Equals(GitLogEntry.Default)) {
      ...
      GUILayout.BeginHorizontal(EditorStyles.toolbar);
      {
        ...
        if (GUILayout.Button(ClearSelectionButton, Styles.ToolbarButtonStyle,
      ...
      DetailsScroll =
          GUILayout.BeginScrollView(DetailsScroll, GUILayout.Height(250));
      {
        ...
        GUILayout.BeginVertical();
        {
          ...
          var treeControlRect = new Rect(
              rect.x + borderLeft, rect.y, Position.width - borderLeft * 2,
          ...
          if (TreeChanges != null) {
            ...
            TreeChanges.FocusedActiveTreeNodeStyle =
                Styles.FocusedActiveTreeNode;

            treeRect = TreeChanges.Render(
                treeControlRect, DetailsScroll, singleClick: node => {},
                doubleClick: node => {}, rightClick: changesTreeRightClick);
...
class HistoryView : HistoryBase {
  [SerializeField]
  private bool currentLogHasUpdate;
  [SerializeField]
  ...
  [SerializeField]
  private CacheUpdateEvent lastLogChangedEvent;
  [SerializeField]
  ...
  [SerializeField]
  private GitLogEntry selectedEntry = GitLogEntry.Default;
  [SerializeField]
  private ChangesTree treeChanges = new ChangesTree { DisplayRootNode = false };
  [SerializeField]
  ...
  protected override void ValidateCachedData(IRepository repository) {
    repository.CheckAndRaiseEventsIfCacheNewer(CacheType.GitLog,
                                               lastLogChangedEvent);
    repository.CheckAndRaiseEventsIfCacheNewer(CacheType.GitAheadBehind,
...
class FileHistoryView : HistoryBase {
  [SerializeField]
  private bool currentFileLogHasUpdate;
  [SerializeField]
  private bool currentStatusEntriesHasUpdate;

  [SerializeField]
  private GitFileLog gitFileLog;
  ...
  private Vector2 detailsScroll;
  [SerializeField]
  private List<GitStatusEntry> gitStatusEntries = new List<GitStatusEntry>();

  [SerializeField]
  private CacheUpdateEvent lastStatusEntriesChangedEvent;
  [SerializeField]
  private CacheUpdateEvent lastFileLogChangedEvent;
  ...
  protected override void ValidateCachedData(IRepository repository) {
    repository.CheckAndRaiseEventsIfCacheNewer(CacheType.GitFileLog,
                                               lastFileLogChangedEvent);
  }
    ...
    DoHistoryGui(lastRect,
                 entry => {
                   ...
                   string checkoutPrompt =
                       string.Format("Checkout revision {0}", entry.ShortID);
                   menu.AddItem(new GUIContent(checkoutPrompt), false,
                                () => Checkout(entry.commitID));
                   menu.ShowAsContext();
                 },
                 node => {});
  }
  ...
  private const string ConfirmCheckoutMessage =
      "You've made changes to file '{0}'.  Overwrite these changes with the historical version?";
  ...
  protected void Checkout(string commitId) {
    var promptUser = gitStatusEntries.Count > 0 &&
                     gitStatusEntries.Any(statusEntry => gitFileLog.Path.Equals(
                                              statusEntry.Path.ToNPath()));

    if (!promptUser ||
        EditorUtility.DisplayDialog(
            ConfirmCheckoutTitle,
            string.Format(ConfirmCheckoutMessage, gitFileLog.Path),
            ConfirmCheckoutOK, ConfirmCheckoutCancel)) {
