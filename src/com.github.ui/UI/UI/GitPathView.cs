...
class GitPathView : Subview {
  ...
  private const string ErrorMinimumGitVersionMessageFormat =
      "Git version {0} found. Git version {1} is required.";
  private const string ErrorMinimumGitLfsVersionMessageFormat =
      "Git LFS version {0} found. Git LFS version {1} is required.";

  [SerializeField]
  private string gitPath;
  [SerializeField]
  private string gitLfsPath;
  [SerializeField]
  private string errorMessage;
  [SerializeField]
  private bool resetToBundled;
  [SerializeField]
  private bool resetToSystem;
  [SerializeField]
  private bool changingManually;
  ...
  [NonSerialized]
  private GitInstaller.GitInstallationState installationState;
  [NonSerialized]
  private GitInstaller.GitInstallDetails installDetails;
  ...
  private void MaybeUpdateData() {
    if (refresh) {
      ...
      installDetails = new GitInstaller.GitInstallDetails(
          Environment.UserCachePath, Environment);
      refresh = false;
    }
  }
  ...
  public override void OnUI() {
    ...
    EditorGUI.BeginDisabledGroup(IsBusy || Parent.IsBusy);
    {
        ...
        GUILayout.BeginHorizontal();
        {
          EditorGUI.BeginChangeCheck();
          {
            ...
            if (GUILayout.Button(BrowseButton, EditorStyles.miniButton,
              ...
              var newPath = EditorUtility.OpenFilePanel(
                  GitInstallBrowseTitle,
                  !String.IsNullOrEmpty(gitLfsPath)
                      ? gitLfsPath.ToNPath().Parent
                      : "",
                  Environment.ExecutableExtension.TrimStart('.'));
      ...
      GUILayout.BeginHorizontal();
      {
        EditorGUI.BeginDisabledGroup(!changingManually && !resetToBundled &&
                                     !resetToSystem);
        {
          if (GUILayout.Button(GitPathSaveButton,
        ...
        // disable the button if the paths are already pointing to the bundled
        // git both on windows, only lfs on mac
        EditorGUI.BeginDisabledGroup(
            (!Environment.IsWindows ||
             gitPath == installDetails.GitExecutablePath) &&
            gitLfsPath == installDetails.GitLfsExecutablePath);
        {
          if (GUILayout.Button(SetToBundledGitButton,
                               GUILayout.ExpandWidth(false))) {
            GUI.FocusControl(null);
        ...
        if (GUILayout.Button(FindSystemGitButton,
                             GUILayout.ExpandWidth(false))) {
          GUI.FocusControl(null);
          isBusy = true;
          new FuncTask<GitInstaller.GitInstallationState>(
              TaskManager.Token,
              () => {
                var gitInstaller = new GitInstaller(
                    Environment, Manager.ProcessManager, TaskManager.Token);
                return gitInstaller.FindSystemGit(
                    new GitInstaller.GitInstallationState());
    ...
    if (resetToBundled) {
      new FuncTask<GitInstaller.GitInstallationState>(
          ...
          () => {
            ...
            var gitInstaller = new GitInstaller(
                Environment, Manager.ProcessManager, TaskManager.Token, state);
      ...
      var installer = new GitInstaller(Environment, Manager.ProcessManager,
                                       TaskManager.Token, newState);
      installer.Progress(UpdateProgress);

      new FuncTask<GitInstaller.GitInstallationState>(
          TaskManager.Token, () => { return installer.RunSynchronously(); })
          .Then((success, state) => {
            if (state.GitIsValid && state.GitLfsIsValid) {
              Manager.SetupGit(state);
              Manager.RestartRepository();
            ...
            if (!state.GitIsValid || !state.GitLfsIsValid) {
              ...
              if (state.GitVersion == TheVersion.Default) {
                ...
                if (state.GitVersion < Constants.MinimumGitVersion) {
                  errorMessageStringBuilder.AppendFormat(
                      ErrorMinimumGitVersionMessageFormat, state.GitVersion,
                ...
                if (state.GitLfsVersion < Constants.MinimumGitLfsVersion) {
                  ...
                  errorMessageStringBuilder.AppendFormat(
                      ErrorMinimumGitLfsVersionMessageFormat,
                      state.GitLfsVersion, Constants.MinimumGitLfsVersion);
                }
              }
              ...
              Logger.Trace("Software versions meet minimums Git:{0} GitLfs:{1}",
                           state.GitVersion, state.GitLfsVersion);
  ...
  public bool ViewHasChanges {
    get {
      return gitPath != installationState.GitExecutablePath ||
             gitLfsPath != installationState.GitLfsExecutablePath;
    }
  }
