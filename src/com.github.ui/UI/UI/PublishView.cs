...
class PublishView : Subview {
  ...
  private const string PrivateRepoMessage =
      "You choose who can see and commit to this repository";
  private const string PublicRepoMessage =
  ...
  private const string PublishLimitPrivateRepositoriesError =
      "You are currently at your limit of private repositories";
  ...
  [SerializeField]
  private string[] owners = { OwnersDefaultText };
  [SerializeField]
  private string[] publishOwners;
  [SerializeField]
  ...
  [NonSerialized]
  private Dictionary<string, IApiClient> clients =
      new Dictionary<string, IApiClient>();
  [NonSerialized]
  private IApiClient selectedClient;
  [NonSerialized]
  private bool isBusy;
  [NonSerialized]
  private string error;
  [NonSerialized]
  private bool connectionsNeedLoading;
  [NonSerialized]
  private bool ownersNeedLoading;
  ...
  private IApiClient GetApiClient(Connection connection) {
    ...
    if (!clients.TryGetValue(connection.Host, out client)) {
      client = new ApiClient(Platform.Keychain, Platform.ProcessManager,
                             TaskManager, Environment, connection.Host);
  ...
  private void LoadOwners() {
    ...
    selectedClient.GetOrganizations(
        orgs => {
          publishOwners = orgs.OrderBy(organization => organization.Login)
                              .Select(organization => organization.Login)
                              .ToArray();

          owners = new[] { OwnersDefaultText,
                           connections[selectedConnection].Username }
                       .Union(publishOwners)
                       .ToArray();
        ...
        exception => {
          ...
          if (keychainEmptyException != null) {
            PopupWindow.OpenWindow(
                PopupWindow.PopupViewType.AuthenticationView);
            return;
          }
  ...
  public override void OnUI() {
    ...
    EditorGUI.BeginDisabledGroup(isBusy);
    {
      if (connections.Length > 1) {
        ...
        {
          selectedConnection = EditorGUILayout.Popup(
              "Connections:", selectedConnection, connectionLabels);
        }
      ...
      selectedOwner =
          EditorGUILayout.Popup(SelectedOwnerLabel, selectedOwner, owners);
      repoName = EditorGUILayout.TextField(RepositoryNameLabel, repoName);
      repoDescription =
          EditorGUILayout.TextField(DescriptionLabel, repoDescription);

      togglePrivate =
          EditorGUILayout.Toggle(CreatePrivateRepositoryLabel, togglePrivate);

      var repoPrivacyExplanation =
          togglePrivate ? PrivateRepoMessage : PublicRepoMessage;
      ...
      GUILayout.BeginHorizontal();
      {
        ...
        if (GUILayout.Button(PublishViewCreateButton)) {
          ...
          var organization =
              owners[selectedOwner] == connections[selectedConnection].Username
                  ? null
                  : owners[selectedOwner];

          var cleanRepoDescription = repoDescription.Trim();
          cleanRepoDescription = string.IsNullOrEmpty(cleanRepoDescription)
                                     ? null
                                     : cleanRepoDescription;

          selectedClient.CreateRepository(
              repoName, cleanRepoDescription,
              togglePrivate, (repository, ex) => {
                if (ex != null) {
                  Logger.Error(ex, "Repository Create Error Type:{0}",
                               ex.GetType().ToString());
  ...
  private bool IsFormValid {
    get {
      return !string.IsNullOrEmpty(repoName) && !isBusy && selectedOwner != 0;
    }
  }
}
}
