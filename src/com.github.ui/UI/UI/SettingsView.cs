...
class SettingsView : Subview {
  ...
  private const string MetricsOptInLabel =
  ...
  private bool isBusy;
  [NonSerialized]
  private bool metricsHasChanged;

  [SerializeField]
  private GitPathView gitPathView = new GitPathView();
  [SerializeField]
  ...
  [SerializeField]
  private string newRepositoryRemoteUrl;
  [SerializeField]
  ...
  private void MaybeUpdateData() {
    if (metricsHasChanged) {
      metricsEnabled =
          Manager.UsageTracker != null ? Manager.UsageTracker.Enabled : false;
    ...
    if (currentRemoteHasUpdate) {
      ...
      hasRemote = currentRemote.HasValue &&
                  !String.IsNullOrEmpty(currentRemote.Value.Url);
      if (!hasRemote) {
  ...
  private void OnRepositorySettingsGUI() {
    ...
    {
      newRepositoryRemoteUrl = EditorGUILayout.TextField(
          GitRepositoryRemoteLabel + ": " + repositoryRemoteName,
