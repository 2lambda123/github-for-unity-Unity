...
class ChangesView : Subview {
  ...
  [SerializeField]
  private bool currentBranchHasUpdate;
  [SerializeField]
  ...
  [NonSerialized]
  private GUIContent discardGuiContent;
  [NonSerialized]
  private bool isBusy;
  ...
  [SerializeField]
  private ChangesTree treeChanges =
      new ChangesTree { DisplayRootNode = false, IsCheckable = true,
                        IsUsingGlobalSelection = true };

  [SerializeField]
  private HashSet<NPath> gitLocks = new HashSet<NPath>();
  [SerializeField]
  private List<GitStatusEntry> gitStatusEntries = new List<GitStatusEntry>();

  [SerializeField]
  private string changedFilesText = NoChangedFilesLabel;

  [SerializeField]
  private CacheUpdateEvent lastCurrentBranchChangedEvent;
  [SerializeField]
  ...
  private void DoButtonBarGUI() {
    GUILayout.BeginHorizontal();
    {
      EditorGUI.BeginDisabledGroup(gitStatusEntries == null ||
                                   gitStatusEntries.Count == 0);
      {
  ...
  private void OnTreeGUI(Rect rect) {
    if (treeChanges != null) {
      ...
      var treeRenderRect =
          treeChanges.Render(rect, treeScroll, node => {}, node => {},
                             node => {
  ...
  private GenericMenu CreateContextMenu(ChangesTreeNode node) {
    ...
    genericMenu.AddItem(new GUIContent("Show Diff"), false, () => {
      ...
      calculateDiff
          .FinallyInUI((s, ex, leftRight) => {
            if (s)
              EditorUtility.InvokeDiffTool(
                  leftRight[0].IsInitialized ? leftRight[0].FileName : null,
                  leftRight[0].IsInitialized
                      ? leftRight[0].MakeAbsolute().ToString()
                      : null,
                  leftRight[1].IsInitialized ? leftRight[1].FileName : null,
                  leftRight[1].IsInitialized
                      ? leftRight[1].MakeAbsolute().ToString()
                      : null,
                  null, null);
            else
              ex.Rethrow();
    ...
    genericMenu.AddItem(discardGuiContent, false, () => {
      ...
      if (node.isFolder) {
        discardEntries = treeChanges.GetLeafNodes(node)
  ...
  private ITask<NPath[]> CalculateFolderDiff(ChangesTreeNode node) {
    ...
    var tmpDir = Manager.Environment.UnityProjectPath.Combine("Temp")
                     .CreateTempDirectory();
    var changedFiles =
        treeChanges.GetLeafNodes(node).Select(x => x.Path.ToNPath()).ToList();
  ...
  private ITask<NPath[]> CalculateFileDiff(ChangesTreeNode node) {
    var rightFile = node.Path.ToNPath();
    var tmpDir =
        Manager.Environment.UnityProjectPath.Combine("Temp", "ghu-diffs")
            .EnsureDirectoryExists();
    var leftFile =
        tmpDir.Combine(rightFile.FileName + "_" + Repository.CurrentHead +
                       rightFile.ExtensionWithDot);
    return new SimpleProcessTask(
               TaskManager.Token,
        ...
        .Then((success, txt) => {
          ...
          var leftFolder =
              tmpDir.Combine("left", leftFile.FileName).EnsureDirectoryExists();
          var rightFolder = tmpDir.Combine("right", leftFile.FileName)
                                .EnsureDirectoryExists();
          // file was deleted
          ...
          // file was created
          if (!success) {
            rightFolder.Combine(rightFile).WriteAllText(
                rightFile.ReadAllText());
          }
          return new NPath[] { leftFolder, rightFolder };
        });
  }
  ...
  private void
  RepositoryOnCurrentBranchChanged(CacheUpdateEvent cacheUpdateEvent) {
  ...
  private void MaybeUpdateData() {
    ...
    if (currentLocksHasUpdate) {
      gitLocks = new HashSet<NPath>(
          Repository.CurrentLocks.Select(gitLock => gitLock.Path));
    }
    ...
    if (currentStatusEntriesHasUpdate) {
      ...
      changedFilesText =
          gitStatusEntries.Count == 0 ? NoChangedFilesLabel
  ...
  private void BuildTree() {
    ...
    treeChanges.Load(gitStatusEntries.Select(
        entry => new GitStatusEntryTreeData(
            entry, gitLocks.Contains(entry.Path.ToNPath()))));
    Redraw();
  }
  ...
  private void DoCommitGUI() {
    GUILayout.BeginHorizontal();
    {
                      ...
                      Styles.CommitAreaMinHeight, Styles.CommitAreaMaxHeight)));
      {
        ...
        commitMessage =
            EditorGUILayout.TextField(commitMessage, Styles.TextFieldStyle);

        GUILayout.Space(Styles.CommitAreaPadding * 2);

        GUILayout.Label(DescriptionLabel);
        commitBody = EditorGUILayout.TextArea(
            commitBody, Styles.CommitDescriptionFieldStyle,
            GUILayout.ExpandHeight(true));

        GUILayout.Space(Styles.CommitAreaPadding);

        // Disable committing when already committing or if we don't have all
        // the data needed
        // Debug.LogFormat("IsBusy:{0} string.IsNullOrEmpty(commitMessage): {1}
        // treeChanges.GetCheckedFiles().Any(): {2}",
        //    IsBusy, string.IsNullOrEmpty(commitMessage),
        //    treeChanges.GetCheckedFiles().Any());
        EditorGUI.BeginDisabledGroup(IsBusy ||
                                     ...
                                     !treeChanges.GetCheckedFiles().Any());
        {
          ...
          {
            GUILayout.FlexibleSpace();
            if (GUILayout.Button(String.Format(CommitButton, currentBranch),
                                 Styles.CommitButtonStyle)) {
  ...
  private void SelectAll() { this.treeChanges.SetCheckStateOnAll(true); }

  private void SelectNone() { this.treeChanges.SetCheckStateOnAll(false); }
  ...
  private void Commit() {
    ...
    if (files.Count == gitStatusEntries.Count) {
      ...
      // if there are files that have been staged outside of Unity, but they
      // aren't selected for commit, remove them from the index before
      // commiting, otherwise the commit will take them along.
      var filesStagedButNotChecked = gitStatusEntries.Where(x => x.Staged)
                                         .Select(x => x.Path)
                                         .Except(files)
                                         .ToList();
