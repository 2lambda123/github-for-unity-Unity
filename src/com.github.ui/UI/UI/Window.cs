...
class Window : BaseWindow {
  ...
  [NonSerialized]
  private IProgress repositoryProgress;
  [NonSerialized]
  private IProgress appManagerProgress;
  [NonSerialized]
  private bool firstOnGUI = true;

  [SerializeField]
  private double progressMessageClearTime = -1;
  [SerializeField]
  private double notificationClearTime = -1;
  [SerializeField]
  private double timeSinceLastRotation = -1f;
  [SerializeField]
  private bool currentBranchAndRemoteHasUpdate;
  [SerializeField]
  private bool currentTrackingStatusHasUpdate;
  [SerializeField]
  private bool currentStatusEntriesHasUpdate;
  [SerializeField]
  private bool repositoryProgressHasUpdate;
  [SerializeField]
  private bool appManagerProgressHasUpdate;
  [SerializeField]
  private SubTab changeTab = SubTab.InitProject;
  [SerializeField]
  private SubTab activeTab = SubTab.InitProject;
  [SerializeField]
  private InitProjectView initProjectView = new InitProjectView();
  [SerializeField]
  private BranchesView branchesView = new BranchesView();
  [SerializeField]
  private ChangesView changesView = new ChangesView();
  [SerializeField]
  private HistoryView historyView = new HistoryView();
  [SerializeField]
  private SettingsView settingsView = new SettingsView();
  [SerializeField]
  private LocksView locksView = new LocksView();
  [SerializeField]
  ...
  [SerializeField]
  private int statusAhead;
  [SerializeField]
  private int statusBehind;
  [SerializeField]
  private bool hasItemsToCommit;
  [SerializeField]
  private bool isTrackingRemoteBranch;
  [SerializeField]
  private GUIContent currentBranchContent;
  [SerializeField]
  private GUIContent currentRemoteUrlContent;
  [SerializeField]
  private CacheUpdateEvent lastCurrentBranchAndRemoteChangedEvent;
  [SerializeField]
  private CacheUpdateEvent lastTrackingStatusChangedEvent;
  [SerializeField]
  private CacheUpdateEvent lastStatusEntriesChangedEvent;

  [SerializeField]
  private GUIContent pullButtonContent =
      new GUIContent(Localization.PullButton);
  [SerializeField]
  private GUIContent pushButtonContent =
      new GUIContent(Localization.PushButton);
  [SerializeField]
  private GUIContent refreshButtonContent =
      new GUIContent(Localization.RefreshButton);
  [SerializeField]
  private GUIContent fetchButtonContent =
      new GUIContent(Localization.FetchButton);
  [SerializeField]
  private float repositoryProgressValue;
  [SerializeField]
  private string repositoryProgressMessage;
  [SerializeField]
  private float appManagerProgressValue;
  [SerializeField]
  private string appManagerProgressMessage;
  [SerializeField]
  private Connection[] connections;
  [SerializeField]
  private string primaryConnectionUsername;

  public static Window GetWindow() {
    return Resources.FindObjectsOfTypeAll(typeof(Window))
        .FirstOrDefault() as Window;
  }
  ...
  private void ValidateCachedData(IRepository repository) {
    repository.CheckAndRaiseEventsIfCacheNewer(
        CacheType.RepositoryInfo, lastCurrentBranchAndRemoteChangedEvent);
  }
  ...
  private void MaybeUpdateData() {
    ...
    if (HasRepository && !string.IsNullOrEmpty(Repository.CloneUrl)) {
      var host = Repository.CloneUrl.ToRepositoryUri().GetComponents(
          UriComponents.Host, UriFormat.SafeUnescaped);

      connections =
          Platform.Keychain.Connections.OrderByDescending(x => x.Host == host)
              .ToArray();
    } else {
      connections = Platform.Keychain.Connections
                        .OrderByDescending(HostAddress.IsGitHubDotCom)
                        .ToArray();
    }
    ...
    if (repositoryProgressHasUpdate) {
      if (repositoryProgress != null) {
        ...
        if (progressMessageClearTime == -1f ||
            progressMessageClearTime <
                EditorApplication.timeSinceStartup + DefaultNotificationTimeout)
          progressMessageClearTime =
              EditorApplication.timeSinceStartup + DefaultNotificationTimeout;
      } else {
    ...
    if (currentStatusEntriesHasUpdate) {
      ...
      hasItemsToCommit =
          currentChanges != null &&
          currentChanges.Any(entry => entry.Status != GitFileStatus.Ignored &&
                                      !entry.Staged);
    }
    ...
    if (Repository != null) {
      if (currentBranch == null || currentRemoteName == null ||
          currentBranchAndRemoteHasUpdate) {
        ...
        if (repositoryCurrentBranch.HasValue) {
          ...
          isTrackingRemoteBranch =
              !string.IsNullOrEmpty(repositoryCurrentBranch.Value.Tracking);
        } else {
    ...
    if (shouldUpdateContentFields || currentBranchContent == null ||
        currentRemoteUrlContent == null) {
      currentBranchContent =
          new GUIContent(currentBranch, Localization.Window_RepoBranchTooltip);

      if (currentRemoteName != null) {
        currentRemoteUrlContent = new GUIContent(
            currentRemoteUrl, string.Format(Localization.Window_RepoUrlTooltip,
                                            currentRemoteName));
      } else {
        currentRemoteUrlContent = new GUIContent(
            currentRemoteUrl, Localization.Window_RepoNoUrlTooltip);
      }
    }
  }
  ...
  private void AttachHandlers(IRepository repository) {
    if (repository == null)
      return;
    repository.CurrentBranchAndRemoteChanged +=
        RepositoryOnCurrentBranchAndRemoteChanged;
  ...
  private void
  RepositoryOnCurrentBranchAndRemoteChanged(CacheUpdateEvent cacheUpdateEvent) {
  ...
  private void
  RepositoryOnTrackingStatusChanged(CacheUpdateEvent cacheUpdateEvent) {
  ...
  private void
  RepositoryOnStatusEntriesChanged(CacheUpdateEvent cacheUpdateEvent) {
  ...
  public override void UpdateProgress(IProgress progress) {
    lock (lck) {
      ...
      if (repositoryProgress != null && progress != null) {
        repositoryProgress.UpdateProgress(progress.Value, progress.Total,
                                          progress.Message);
      }
  ...
  public override void Update() {
    ...
    if (notificationClearTime > 0f &&
        EditorApplication.timeSinceStartup > notificationClearTime) {
    ...
    // Notification auto-clear timer override
    if (progressMessageClearTime > 0f &&
        EditorApplication.timeSinceStartup > progressMessageClearTime) {
      repositoryProgressHasUpdate = true;
      ClearProgressMessage();
  ...
  public override void DoProgressGUI() {
    ...
    if (Event.current.GetTypeForControl(GUIUtility.GetControlID(
            "ghu_ProgressBar".GetHashCode(), FocusType.Keyboard, position)) ==
        EventType.Repaint) {
      var style = Styles.ProgressAreaBackStyle;
      style.Draw(rect1, false, false, false, false);
      Rect rect2 =
          new Rect(rect1.x, rect1.y, position.width * repositoryProgressValue,
                   rect1.height);
  ...
  private void DoHeaderGUI() {
    GUILayout.BeginHorizontal(Styles.HeaderBoxStyle);
    {
      ...
      {
        GUILayout.Space(9);
        GUILayout.Label(Styles.RepoIcon, GUILayout.Height(20),
                        GUILayout.Width(20));
      }
      GUILayout.EndVertical();
  ...
  private void DoToolbarGUI() {
    ...
    GUILayout.BeginHorizontal(EditorStyles.toolbar);
    {
      EditorGUI.BeginChangeCheck();
      {
        if (HasRepository) {
          changeTab =
              TabButton(SubTab.Changes, Localization.ChangesTitle, changeTab);
          changeTab =
              TabButton(SubTab.Locks, Localization.LocksTitle, changeTab);
          changeTab =
              TabButton(SubTab.History, Localization.HistoryTitle, changeTab);
          changeTab =
              TabButton(SubTab.Branches, Localization.BranchesTitle, changeTab);
        } else if (!HasRepository) {
          changeTab = TabButton(SubTab.InitProject,
                                Localization.InitializeTitle, changeTab);
        }
        changeTab =
            TabButton(SubTab.Settings, Localization.SettingsTitle, changeTab);
      }
      ...
      if (!HasRepository) {
        ...
        if (!connections.Any()) {
          ...
          if (GUILayout.Button(primaryConnectionUsername,
    ...
    GUILayout.BeginHorizontal(EditorStyles.toolbar);
    {
      if (hasRemote) {
        EditorGUI.BeginDisabledGroup(currentRemoteName == null);
        {
          // Fetch button
          var fetchClicked =
              GUILayout.Button(fetchButtonContent, Styles.ToolbarButtonStyle);
          if (fetchClicked) {
            Fetch();
          }
          ...
          var pullButtonText =
              statusBehind > 0
                  ? new GUIContent(String.Format(Localization.PullButtonCount,
                                                 statusBehind))
                  : pullButtonContent;
          var pullClicked =
              GUILayout.Button(pullButtonText, Styles.ToolbarButtonStyle);

          if (pullClicked &&
              EditorUtility.DisplayDialog(
                  Localization.PullConfirmTitle,
                  String.Format(Localization.PullConfirmDescription,
                                currentRemoteName),
                  Localization.PullConfirmYes, Localization.Cancel)) {
            Pull();
          }
        }
        ...
        {
          var pushButtonText =
              statusAhead > 0 ? new GUIContent(String.Format(
                                    Localization.PushButtonCount, statusAhead))
                              : pushButtonContent;
          var pushClicked =
              GUILayout.Button(pushButtonText, Styles.ToolbarButtonStyle);

          if (pushClicked &&
              EditorUtility.DisplayDialog(
                  Localization.PushConfirmTitle,
                  String.Format(Localization.PushConfirmDescription,
                                currentRemoteName),
        ...
        // Publishing a repo
        if (GUILayout.Button(Localization.PublishButton,
      ...
      if (!connections.Any()) {
        ...
        var connection = connections.First();
        if (GUILayout.Button(connection.Username,
                             EditorStyles.toolbarDropDown)) {
          DoAccountDropdown();
        }
      }
  ...
  private void DoActiveViewGUI() {
    ...
    if (IsBusy && activeTab != SubTab.Settings &&
      ...
      if (timeSinceLastRotation < 0) {
        ...
        var elapsedTime =
            (float)(EditorApplication.timeSinceStartup - timeSinceLastRotation);
        if (spinner == null)
          spinner = new Spinner();
        ...
        rect = new Rect(0f, rect.y + rect.height, Position.width,
                        Position.height - (rect.height + rect.y));
        rect = spinner.Layout(rect);
        rect.y += rect.height + 30;
        rect.height = 20;
        if (!String.IsNullOrEmpty(appManagerProgressMessage))
          EditorGUI.ProgressBar(rect, appManagerProgressValue,
                                appManagerProgressMessage);
      }
    }
  }
  ...
  public override void DoEmptyGUI() {
    ...
    {
      GUILayout.FlexibleSpace();
      GUILayout.Label(Styles.EmptyStateInit, GUILayout.MaxWidth(265),
                      GUILayout.MaxHeight(136));
      GUILayout.FlexibleSpace();
    }
    GUILayout.EndHorizontal();
    GUILayout.FlexibleSpace();
  ...
  private void Pull() {
    if (hasItemsToCommit) {
      EditorUtility.DisplayDialog(
          "Pull", "You need to commit your changes before pulling.", "Cancel");
    } else {
      ...
      Repository.Pull()
          .FinallyInUI((success, e) => {
            if (success) {
              ...
              EditorUtility.DisplayDialog(
                  Localization.PullActionTitle,
                  String.Format(Localization.PullSuccessDescription,
                                currentRemoteName),
                  Localization.Ok);
    ...
    Repository.Push()
        .FinallyInUI((success, e) => {
          if (success) {
            ...
            EditorUtility.DisplayDialog(
                Localization.PushActionTitle,
                String.Format(Localization.PushSuccessDescription,
                              currentRemoteName),
                Localization.Ok);
          } else {
            SetProgressMessage(Localization.MessagePushFailed, 100);
            EditorUtility.DisplayDialog(Localization.PushActionTitle, e.Message,
                                        Localization.Ok);
          }
        })
        .Start();
    ...
    Repository.Fetch()
        .FinallyInUI((success, e) => {
          if (success) {
            ...
            EditorUtility.DisplayDialog(Localization.FetchActionTitle,
                                        e.Message, Localization.Ok);
          }
        })
        .Start();
  ...
  private void DoAccountDropdown() {
    ...
    if (connections.Length == 1) {
      var connection = connections.First();
      accountMenu.AddItem(new GUIContent("Go to Profile"), false, GoToProfile,
                          connection);
      accountMenu.AddItem(new GUIContent("Sign out"), false, SignOut,
                          connection);
      ...
      for (var index = 0; index < connections.Length; index++) {
        ...
        accountMenu.AddItem(new GUIContent(rootPath + "Go to Profile"), false,
                            GoToProfile, connection);
        accountMenu.AddItem(new GUIContent(rootPath + "Sign out"), false,
                            SignOut, connection);
      }
    }
  ...
  private void SignOut(object obj) {
    var connection = (Connection)obj;
    var loginManager =
        new LoginManager(Platform.Keychain, Manager.ProcessManager,
                         Manager.TaskManager, Environment);
...
public void ShowNotification(GUIContent content, float timeout) {
  notificationClearTime = timeout < DefaultNotificationTimeout
...
private static SubTab TabButton(SubTab tab, string title, SubTab currentTab) {
  return GUILayout.Toggle(currentTab == tab, title, EditorStyles.toolbarButton)
             ? tab
             : currentTab;
}
