...
class BranchesView : Subview {
  ...
  private const string ConfirmCheckoutBranchMessage =
      "Checkout branch {0} from {1}?";
  private const string ConfirmCheckoutBranchOK = "Checkout";
  private const string ConfirmCheckoutBranchCancel = "Cancel";
  private const string WarningCheckoutBranchExistsTitle =
      "Branch already exists";
  private const string WarningCheckoutBranchExistsMessage =
  ...
  private const string DeleteBranchMessageFormatString =
  ...
  [NonSerialized]
  private BranchesMode targetMode;

  [SerializeField]
  private BranchesTree treeLocals = new BranchesTree { Title = LocalTitle };
  [SerializeField]
  private BranchesTree treeRemotes =
      new BranchesTree { Title = RemoteTitle, IsRemote = true };
  [SerializeField]
  private BranchesMode mode = BranchesMode.Default;
  [SerializeField]
  ...
  [SerializeField]
  private CacheUpdateEvent lastLocalAndRemoteBranchListChangedEvent;
  [NonSerialized]
  private bool localAndRemoteBranchListHasUpdate;

  [SerializeField]
  private CacheUpdateEvent lastCurrentBranchAndRemoteChange;
  [NonSerialized]
  private bool currentBranchAndRemoteChangeHasUpdate;

  [SerializeField]
  private GitBranch currentBranch = GitBranch.Default;
  [SerializeField]
  private GitRemote currentRemote = GitRemote.Default;
  ...
  public override void OnBeforeFirstDraw() {
    ...
    if (treeLocals != null) {
      ...
      treeLocals.UpdateIcons(Styles.ActiveBranchIcon, Styles.BranchIcon,
                             Styles.FolderIcon, Styles.GlobeIcon);
    }

    if (treeRemotes != null) {
      treeRemotes.ViewHasFocus = hasFocus;
      treeRemotes.UpdateIcons(Styles.ActiveBranchIcon, Styles.BranchIcon,
                              Styles.FolderIcon, Styles.GlobeIcon);
    }
  ...
  public override void OnFocusChanged() {
    base.OnFocusChanged();
    if (treeLocals.ViewHasFocus != HasFocus ||
        treeRemotes.ViewHasFocus != HasFocus) {
      treeLocals.ViewHasFocus = HasFocus;
      treeRemotes.ViewHasFocus = HasFocus;
      Redraw();
  ...
  private void RepositoryOnLocalAndRemoteBranchListChanged(
  ...
  private void MaybeUpdateData() {
    ...
    if (currentBranchAndRemoteChangeHasUpdate ||
        localAndRemoteBranchListHasUpdate) {
    ...
    disableDelete = treeLocals.SelectedNode == null ||
                    treeLocals.SelectedNode.IsFolder ||
                    treeLocals.SelectedNode.IsActive;
    disableCreate = treeLocals.SelectedNode == null ||
                    treeLocals.SelectedNode.IsFolder ||
                    treeLocals.SelectedNode.Level == 0;
  }

  public override void OnUI() { Render(); }

  private void AttachHandlers(IRepository repository) {
    repository.LocalAndRemoteBranchListChanged +=
        RepositoryOnLocalAndRemoteBranchListChanged;
  ...
  private void ValidateCachedData(IRepository repository) {
    repository.CheckAndRaiseEventsIfCacheNewer(
        CacheType.Branches, lastLocalAndRemoteBranchListChangedEvent);
    repository.CheckAndRaiseEventsIfCacheNewer(
        CacheType.RepositoryInfo, lastCurrentBranchAndRemoteChange);
  }
  ...
  private void BuildTree() {
    ...
    treeLocals.Load(localBranches.Select(
        branch => new GitBranchTreeData(
            branch, currentBranch != GitBranch.Default &&
                        currentBranch.Name == branch.Name)));
    treeRemotes.Load(
        remoteBranches.Select(branch => new GitBranchTreeData(branch, false)));
    Redraw();
  }
  ...
  private void OnButtonBarGUI() {
    if (mode == BranchesMode.Default) {
      ...
      {
        if (GUILayout.Button(DeleteBranchButton, EditorStyles.miniButton,
      ...
      {
        if (GUILayout.Button(CreateBranchButton, EditorStyles.miniButton,
                             GUILayout.ExpandWidth(false))) {
    ...
    // Branch name + cancel + create
      ...
      GUILayout.BeginHorizontal();
      {
        ...
        {
          if (GUILayout.Button(NewBranchConfirmButton,
        ...
        // Cancel create
        if (GUILayout.Button(NewBranchCancelButton,
          ...
          Repository.CreateBranch(newBranchName, treeLocals.SelectedNode.Path)
              .FinallyInUI((success, e) => {
                if (success) {
                  ...
                  var errorMessage =
                      e.Message.StartsWith(errorHeader)
                          ? e.Message.Remove(0, errorHeader.Length)
                          : e.Message;

                  EditorUtility.DisplayDialog(CreateBranchTitle, errorMessage,
                                              Localization.Ok);
                }
              })
              .Start();
  ...
  private void OnTreeGUI(Rect rect) {
    ...
    if (treeLocals != null && treeRemotes != null) {
      ...
      treeRenderRect =
          treeLocals.Render(rect, scroll, node => {},
                            node => {
                              if (node.IsFolder)
                                return;
                            ...
                            node => {
                              ...
                              var menu =
                                  CreateContextMenuForLocalBranchNode(node);
                              menu.ShowAsContext();
                            });
      ...
      var treeRemoteDisplayRect =
          new Rect(rect.x, treeRenderRect.y, rect.width, rect.height);
      treeRenderRect =
          treeRemotes.Render(treeRemoteDisplayRect, scroll, node => {},
                             node => {
                               if (node.IsFolder)
                                 return;
                             ...
                             node => {
                               ...
                               var menu =
                                   CreateContextMenuForRemoteBranchNode(node);
                               menu.ShowAsContext();
                             });
  ...
  private void CheckoutRemoteBranch(string branch) {
    ...
    if (Repository.LocalBranches.Any(localBranch =>
                                         localBranch.Name == branchName)) {
      ...
      var confirmCheckout = EditorUtility.DisplayDialog(
          ConfirmCheckoutBranchTitle,
          String.Format(ConfirmCheckoutBranchMessage, branch, originName),
        ...
        Repository.CreateBranch(branchName, branch)
            .FinallyInUI((success, e) => {
              if (success) {
                ...
                EditorUtility.DisplayDialog(
                    Localization.SwitchBranchTitle,
                    String.Format(Localization.SwitchBranchFailedDescription,
                                  branch),
                    Localization.Ok);
              }
            })
  ...
  private void SwitchBranch(string branch) {
    if (EditorUtility.DisplayDialog(ConfirmSwitchTitle,
                                    String.Format(ConfirmSwitchMessage, branch),
                                    ConfirmSwitchOK, ConfirmSwitchCancel)) {
  ...
  private void DeleteLocalBranch(string branch) {
    ...
    if (EditorUtility.DisplayDialog(DeleteBranchTitle, dialogMessage,
                                    DeleteBranchButton, CancelButtonLabel)) {
      Repository.DeleteBranch(branch, true)
          .Finally(s => {
            if (s)
              UsageTracker.IncrementBranchesViewButtonDeleteBranch();
          })
          .Start();
    }
  }
  ...
  private enum NodeType { Folder, LocalBranch, RemoteBranch }

  private enum BranchesMode { Default, Create }
}
}
