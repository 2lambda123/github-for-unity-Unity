...
[Serializable]
public class GitLockEntryDictionary
    : SerializableDictionary<string, GitLockEntry> {}
...
public class GitLockEntry {
  public static GitLockEntry Default =
      new GitLockEntry(GitLock.Default, GitFileStatus.None);

  [NonSerialized]
  public Texture Icon;
  [NonSerialized]
  public Texture IconBadge;
  ...
  public GitLockEntry(GitLock gitLock, GitFileStatus gitFileStatus) {
    ...
    this.lockedAt =
        gitLock.LockedAt.ToLocalTime().CreateRelativeTime(DateTimeOffset.Now);
  }
...
class LocksControl {
  [NonSerialized]
  private Action<GitLock> rightClickNextRender;
  [NonSerialized]
  private GitLockEntry rightClickNextRenderEntry;
  [NonSerialized]
  private int controlId;
  [NonSerialized]
  private UnityEngine.Object lastActivatedObject;
  [NonSerialized]
  private Dictionary<string, bool> visibleItems =
      new Dictionary<string, bool>();

  [SerializeField]
  private Vector2 scroll;
  [SerializeField]
  private List<GitLockEntry> gitLockEntries = new List<GitLockEntry>();
  [SerializeField]
  public GitLockEntryDictionary assets = new GitLockEntryDictionary();
  [SerializeField]
  public GitStatusDictionary gitStatusDictionary = new GitStatusDictionary();
  [SerializeField]
  private GitLockEntry selectedEntry;
  [SerializeField]
  public NPath projectPath;

  public bool IsEmpty {
    get { return gitLockEntries.Count == 0; }
  }
  ...
  public GitLockEntry SelectedEntry {
    ...
    set {
      ...
      var activeObject =
          selectedEntry != null && selectedEntry.GitLock != GitLock.Default &&
                  projectPath.IsInitialized
              ? AssetDatabase.LoadMainAssetAtPath(
                    selectedEntry.GitLock.Path.MakeAbsolute().RelativeTo(
                        projectPath))
              : null;
  ...
  public bool Render(Rect containingRect, Action<GitLock> singleClick = null,
                     Action<GitLock> doubleClick = null,
                     Action<GitLock> rightClick = null) {
    ...
    scroll = GUILayout.BeginScrollView(scroll);
    {
      ...
      for (var index = 0; index < gitLockEntries.Count; index++) {
        ...
        var entryRect =
            new Rect(rect.x, rect.y, rect.width, Styles.LocksEntryHeight);
        ...
        if (visibleItems.ContainsKey(entry.GitLock.ID) &&
            visibleItems[entry.GitLock.ID]) {
  ...
  private Rect RenderEntry(Rect entryRect, GitLockEntry entry) {
    ...
    GUILayout.Label(entry.Icon, GUILayout.Height(iconWidth),
                    GUILayout.Width(iconHeight));
    if (Event.current.type == EventType.Repaint) {
      var iconRect = GUILayoutUtility.GetLastRect();
      var iconBadgeRect =
          new Rect(iconRect.x + iconBadgeWidth, iconRect.y + iconBadgeHeight,
                   iconBadgeWidth, iconBadgeHeight);
      Styles.Label.Draw(iconBadgeRect, entry.IconBadge, false, false, false,
                        hasKeyboardFocus);
    }
    GUILayout.BeginVertical();
    GUILayout.Label(entry.GitLock.Path,
                    isSelected ? Styles.SelectedLabel : Styles.Label);
    GUILayout.Label(string.Format("Locked {0} by {1}", entry.LockedAt,
                                  entry.GitLock.Owner.Name),
                    isSelected ? Styles.LocksViewLockedBySelectedStyle
                               : Styles.LocksViewLockedByStyle);
  ...
  private bool HandleInput(Rect rect, GitLockEntry entry, int index,
                           Action<GitLock> singleClick = null,
                           Action<GitLock> doubleClick = null,
                           Action<GitLock> rightClick = null) {
    var requiresRepaint = false;
    ...
    if (GUIUtility.keyboardControl == controlId && entry == SelectedEntry &&
  ...
  public void Load(List<GitLock> locks, List<GitStatusEntry> gitStatusEntries) {
    ...
    for (int i = 0; i < gitStatusEntries.Count; i++)
      statusEntries.Add(
          gitStatusEntries[i].Path.ToNPath().ToString(SlashMode.Forward), i);
    var selectedLockId =
        SelectedEntry != null && SelectedEntry.GitLock != GitLock.Default
    ...
    gitLockEntries =
        locks
            .Select(gitLock => {
              ...
              if (statusEntries.TryGetValue(
                      gitLock.Path.ToString(SlashMode.Forward), out index)) {
  ...
  private void LoadIcon(GitLockEntry gitLockEntry, bool force = false) {
    ...
    if (force || gitLockEntry.IconBadge == null) {
      gitLockEntry.IconBadge =
          Styles.GetFileStatusIcon(gitLockEntry.GitFileStatus, true);
    }
  }
  ...
  protected Texture GetNodeIcon(GitLock node) {
    ...
    if (!string.IsNullOrEmpty(node.Path)) {
      nodeIcon =
          UnityEditorInternal.InternalEditorUtility.GetIconForFile(node.Path);
    }
...
class LocksView : Subview {
  ...
  [SerializeField]
  private bool currentRemoteHasUpdate;
  [SerializeField]
  private bool currentStatusEntriesHasUpdate;
  [SerializeField]
  private bool currentLocksHasUpdate;
  [SerializeField]
  private bool keychainHasUpdate;
  [SerializeField]
  private LocksControl locksControl;
  [SerializeField]
  private CacheUpdateEvent lastCurrentRemoteChangedEvent;
  [SerializeField]
  private CacheUpdateEvent lastLocksChangedEvent;
  [SerializeField]
  private CacheUpdateEvent lastStatusEntriesChangedEvent;
  [SerializeField]
  private List<GitLock> lockedFiles = new List<GitLock>();
  [SerializeField]
  private List<GitStatusEntry> gitStatusEntries = new List<GitStatusEntry>();
  [SerializeField]
  private string currentUsername;
  [SerializeField]
  private GUIContent unlockFileMenuContent =
      new GUIContent(Localization.UnlockFileMenuItem);
  [SerializeField]
  private GUIContent forceUnlockFileMenuContent =
      new GUIContent(Localization.ForceUnlockFileMenuItem);
  ...
  public override void OnUI() {
    ...
    if (locksControl != null && !locksControl.IsEmpty) {
      var lockControlRect = new Rect(rect.x, rect.y, Position.width,
                                     Position.height - rect.height);

      var requiresRepaint = locksControl.Render(
          lockControlRect, entry => {}, entry => {},
          entry => {
            var menu = new GenericMenu();
            if (entry.Owner.Name == currentUsername) {
              menu.AddItem(unlockFileMenuContent, false, UnlockSelectedEntry);
            }
            menu.AddItem(forceUnlockFileMenuContent, false,
                         ForceUnlockSelectedEntry);
            menu.ShowAsContext();
          });
  ...
  private void UnlockSelectedEntry() {
    isBusy = true;
    Repository.ReleaseLock(locksControl.SelectedEntry.GitLock.Path, false)
  ...
  private void ForceUnlockSelectedEntry() {
    isBusy = true;
    Repository.ReleaseLock(locksControl.SelectedEntry.GitLock.Path, true)
        .FinallyInUI((success, ex) => {
          if (success) {
            Manager.UsageTracker.IncrementUnityProjectViewContextLfsUnlock();
          } else {
  ...
  private void
  RepositoryOnCurrentRemoteChanged(CacheUpdateEvent cacheUpdateEvent) {
  ...
  private void ValidateCachedData(IRepository repository) {
    repository.CheckAndRaiseEventsIfCacheNewer(CacheType.RepositoryInfo,
                                               lastCurrentRemoteChangedEvent);
    repository.CheckAndRaiseEventsIfCacheNewer(CacheType.GitLocks,
                                               lastLocksChangedEvent);
    repository.CheckAndRaiseEventsIfCacheNewer(CacheType.GitStatus,
    ...
    if (keychainHasUpdate || currentRemoteHasUpdate) {
      ...
      if (Repository != null) {
        ...
        if (!string.IsNullOrEmpty(Repository.CloneUrl)) {
          ...
          connection =
              Platform.Keychain.Connections.FirstOrDefault(x => x.Host == host);
        } else {
          connection = Platform.Keychain.Connections.FirstOrDefault(
              HostAddress.IsGitHubDotCom);
        }
