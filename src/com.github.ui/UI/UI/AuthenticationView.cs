...
class AuthenticationView : Subview {
  ...
  [SerializeField]
  private SubTab changeTab = SubTab.GitHub;
  [SerializeField]
  private SubTab activeTab = SubTab.GitHub;

  [SerializeField]
  private GitHubAuthenticationView gitHubAuthenticationView;
  [SerializeField]
  private GitHubEnterpriseAuthenticationView gitHubEnterpriseAuthenticationView;
  [SerializeField]
  private bool hasGitHubDotComConnection;
  [SerializeField]
  private bool hasGitHubEnterpriseConnection;
  ...
  public override void InitializeView(IView parent) {
    ...
    gitHubAuthenticationView =
        gitHubAuthenticationView ?? new GitHubAuthenticationView();
    gitHubEnterpriseAuthenticationView =
        gitHubEnterpriseAuthenticationView ??
        new GitHubEnterpriseAuthenticationView();
    ...
    hasGitHubDotComConnection =
        Platform.Keychain.Connections.Any(HostAddress.IsGitHubDotCom);
    hasGitHubEnterpriseConnection = Platform.Keychain.Connections.Any(
        connection => !HostAddress.IsGitHubDotCom(connection));
  ...
  public override bool IsBusy {
    get {
      return (gitHubAuthenticationView != null &&
              gitHubAuthenticationView.IsBusy) ||
             (gitHubEnterpriseAuthenticationView != null &&
              gitHubEnterpriseAuthenticationView.IsBusy);
    }
  }
  ...
  private static SubTab TabButton(SubTab tab, string title, SubTab currentTab) {
    return GUILayout.Toggle(currentTab == tab, title,
                            EditorStyles.toolbarButton)
               ? tab
               : currentTab;
  }

  private enum SubTab { None, GitHub, GitHubEnterprise }
  ...
  private void DoToolbarGUI() {
    GUILayout.BeginHorizontal(EditorStyles.toolbar);
    {
      EditorGUI.BeginChangeCheck();
      {
        ...
        {
          changeTab = TabButton(SubTab.GitHubEnterprise, "GitHub Enterprise",
                                changeTab);
        }
        EditorGUI.EndDisabledGroup();
      }
