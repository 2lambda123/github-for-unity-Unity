...
public class FileHistoryWindow : BaseWindow {
  ...
  private static void GitFileHistory() {
    if (Selection.assetGUIDs != null) {
      var assetPath =
          AssetDatabase.GUIDToAssetPath(Selection.assetGUIDs.First()).ToNPath();
  ...
  private bool locked;
  [SerializeField]
  private FileHistoryView fileHistoryView = new FileHistoryView();
  [SerializeField]
  private UnityEngine.Object selectedObject;
  [SerializeField]
  private string selectedObjectAssetPath;
  [SerializeField]
  private string selectedObjectRepositoryPath;
  ...
  public void SetSelectedPath(string assetPath) {
    ...
    if (selectedObjectAssetPath != NPath.Default) {
      ...
      selectedObject =
          AssetDatabase.LoadMainAssetAtPath(selectedObjectAssetPath);
    ...
    Repository.UpdateFileLog(selectedObjectRepositoryPath).Start();
  }
  ...
  private void DoHeaderGUI() {
    GUILayout.BeginHorizontal(Styles.HeaderBoxStyle);
    {
      ...
      GUILayout.Label(selectedIcon, GUILayout.Height(iconWidth),
                      GUILayout.Width(iconHeight));
      GUILayout.Space(16);

      GUILayout.BeginVertical();
      {
        GUILayout.Label(selectedObjectAssetPath,
                        Styles.FileHistoryLogTitleStyle);
