...
class InitProjectView : Subview {
  private const string NoRepoTitle =
      "To begin using GitHub, initialize a git repository";
  private const string NoUserOrEmailError =
  ...
  [SerializeField]
  private bool isUserDataPresent;

  [SerializeField]
  private CacheUpdateEvent lastCheckUserChangedEvent;

  [NonSerialized]
  private bool userHasChanges;
  [NonSerialized]
  private bool gitExecutableIsSet;
  ...
  public override void OnEnable() {
    ...
    User.CheckAndRaiseEventsIfCacheNewer(CacheType.GitUser,
  ...
  public override void OnUI() {
    GUILayout.BeginVertical(Styles.GenericBoxStyle);
    {
      ...
      GUILayout.BeginHorizontal();
      {
        ...
        EditorGUI.BeginDisabledGroup(!gitExecutableIsSet || IsBusy ||
                                     !isUserDataPresent);
        {
          if (GUILayout.Button(Localization.InitializeRepositoryButtonText,
                               "Button")) {
      ...
      if (gitExecutableIsSet && hasCompletedInitialCheck &&
          !isUserDataPresent) {
  ...
  private void AttachHandlers() { User.Changed += UserOnChanged; }
  ...
  private void DetachHandlers() { User.Changed -= UserOnChanged; }
