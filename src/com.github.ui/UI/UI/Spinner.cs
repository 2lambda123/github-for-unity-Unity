...
class Spinner {
  struct Rotation {
    ...
    public override string ToString() {
      return string.Format("rotation:{0} center:{1}:{2}", rotation, center.x,
                           center.y);
    }
  }
  ...
  private Texture2D Inside {
    get { return Styles.SpinnerInside; }
  }
  private Texture2D Outside {
    get { return Styles.SpinnerOutside; }
  }
  private Texture2D Code {
    get { return Styles.Code; }
  }
  private Texture2D Merge {
    get { return Styles.Merge; }
  }
  private Texture2D Rocket {
    get { return Styles.Rocket; }
  }
  ...
  public float Rotate(float currentTime) {
    ...
    currentRotation +=
        deltaTime * speed * ((Mathf.Sin(currentTime * 1.2f)) + 2);
  ...
  public Rect Layout(Rect rect) {
    ...
    outsideRect =
        new Rect(centerX - Outside.width / 2, centerY - Outside.height / 2 - 11,
                 Outside.width, Outside.height);
    codeIconTopLeft = new Vector2(centerX - Code.width / 2 - 0.6f,
                                  centerY - Code.height / 2 - 45);
    mergeIconTopLeft = new Vector2(centerX - Merge.width / 2 + 38,
                                   centerY - Merge.height / 2 + 24);
    rocketIconTopLeft = new Vector2(centerX - Rocket.width / 2 - 39,
                                    centerY - Rocket.height / 2 + 24);
    backgroundRect = new Rect(x, y, width, height);
    insideRect =
        new Rect(centerX - Inside.width / 2, centerY - Inside.height / 2,
                 Inside.width, Inside.height);
    codeIconCenter = new Vector2(codeIconTopLeft.x + Code.width / 2 - .1f,
                                 codeIconTopLeft.y + Code.height / 2 + 0.1f);
    rocketIconCenter = new Vector2(rocketIconTopLeft.x + Rocket.width / 2,
                                   rocketIconTopLeft.y + Rocket.height / 2);
    mergeIconCenter = new Vector2(mergeIconTopLeft.x + Merge.width / 2,
                                  mergeIconTopLeft.y + Merge.height / 2);
    return outsideRect;
  }
  ...
  public void Render() {
    ...
    // draw the background
    GUI.DrawTexture(backgroundRect, Utility.GetTextureFromColor(
    ...
    PushRotation(currentRotation, center);
    GUI.DrawTexture(
        new Rect(codeIconTopLeft.x, codeIconTopLeft.y, Code.width, Code.height),
        Code);
    PopRotation();
    PopRotation();
    ...
    PushRotation(currentRotation, center);
    GUI.DrawTexture(new Rect(mergeIconTopLeft.x, mergeIconTopLeft.y,
    ...
    PushRotation(currentRotation, center);
    GUI.DrawTexture(new Rect(rocketIconTopLeft.x, rocketIconTopLeft.y,
