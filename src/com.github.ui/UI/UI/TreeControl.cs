...
public abstract class Tree<TNode, TData> : TreeBase<TNode, TData>
  ...
  [NonSerialized]
  public GUIStyle FolderStyle;
  [NonSerialized]
  public GUIStyle TreeNodeStyle;
  [NonSerialized]
  public GUIStyle ActiveTreeNodeStyle;
  [NonSerialized]
  public GUIStyle FocusedTreeNodeStyle;
  [NonSerialized]
  public GUIStyle FocusedActiveTreeNodeStyle;

  [NonSerialized]
  private Stack<bool> indents = new Stack<bool>();
  [NonSerialized]
  private Action<TNode> rightClickNextRender;
  [NonSerialized]
  private TNode rightClickNextRenderNode;
  ...
  public bool IsInitialized {
    get {
      return Nodes != null && Nodes.Count > 0 &&
             !String.IsNullOrEmpty(Nodes[0].Path);
    }
  }
  public bool RequiresRepaint { get; private set; }

  public Rect Render(Rect treeDisplayRect, Vector2 scroll,
                     Action<TNode> singleClick = null,
                     Action<TNode> doubleClick = null,
                     Action<TNode> rightClick = null) {
    ...
    var rect = new Rect(treeDisplayRect.x, treeDisplayRect.y,
                        treeDisplayRect.width, ItemHeight);
    ...
    if (DisplayRootNode) {
      ...
      if (titleDisplay) {
        var isSelected =
            SelectedNode != null && SelectedNode.Path == titleNode.Path;
        renderResult =
            titleNode.Render(rect, Styles.TreeIndentation, isSelected, IsBusy,
                             FolderStyle, treeNodeStyle, activeTreeNodeStyle);
      }
    ...
    for (; i < Nodes.Count; i++) {
      ...
      if (display) {
        ...
        renderResult =
            node.Render(rect, Styles.TreeIndentation, isSelected, IsBusy,
                        FolderStyle, treeNodeStyle, activeTreeNodeStyle);
      }
      ...
      if (!node.IsHidden) {
        RequiresRepaint =
            HandleInput(rect, node, i, singleClick, doubleClick, rightClick);
        rect.y += ItemHeight + ItemSpacing;
      }
    }
  ...
  public void Focus() {
    ...
    if (IsSelectable && Event.current.type == EventType.KeyDown) {
      int directionY = Event.current.keyCode == KeyCode.UpArrow     ? -1
                       : Event.current.keyCode == KeyCode.DownArrow ? 1
                                                                    : 0;
      int directionX = Event.current.keyCode == KeyCode.LeftArrow    ? -1
                       : Event.current.keyCode == KeyCode.RightArrow ? 1
                                                                     : 0;
  ...
  private bool HandleInput(Rect rect, TNode currentNode, int index,
                           Action<TNode> singleClick = null,
                           Action<TNode> doubleClick = null,
                           Action<TNode> rightClick = null) {
    ...
    if (IsSelectable && GUIUtility.keyboardControl == controlId &&
  ...
  private void Indent() { indents.Push(true); }

  private void Unindent() { indents.Pop(); }
...
public class TreeNode : ITreeNode {
  ...
  public void Load() { content = new GUIContent(Label, Icon); }

  public TreeNodeRenderResult Render(Rect rect, float indentation,
                                     bool isSelected, bool treeIsBusy,
                                     GUIStyle toggleStyle, GUIStyle nodeStyle,
    ...
    var nodeRect =
        new Rect(nodeStartX, rect.y, fillRect.width - nodeStartX, rect.height);

    var reserveToggleSpace = TreeIsCheckable || isFolder;
    var toggleRect =
        new Rect(nodeStartX, nodeRect.y, reserveToggleSpace ? indentation : 0,
                 nodeRect.height);
    ...
    var checkRect =
        new Rect(nodeStartX, nodeRect.y, TreeIsCheckable ? indentation : 0,
                 nodeRect.height);
    ...
    var iconRect = new Rect(nodeStartX, nodeRect.y, fillRect.width - nodeStartX,
                            nodeRect.height);
    ...
    if (Event.current.type == EventType.Repaint) {
      contentStyle.Draw(fillRect, GUIContent.none, false, false, false,
                        isSelected);
    }
    ...
    if (IsFolderOrContainer) {
      ...
      if (Event.current.type == EventType.Repaint) {
        toggleStyle.Draw(toggleRect, GUIContent.none, false, false, styleOn,
                         isSelected);
      }
  ...
  public override string ToString() {
    return String.Format(
        "path:{0} label:{1} level:{2} isFolder:{3} isCollapsed:{4} isHidden:{5} isActive:{6}",
...
public class BranchesTree : Tree<TreeNode, GitBranchTreeData> {
  ...
  [SerializeField]
  public TreeNodeDictionary folders = new TreeNodeDictionary();
  [SerializeField]
  public TreeNodeDictionary checkedFileNodes = new TreeNodeDictionary();
  ...
  [SerializeField]
  public bool displayRootNode = true;
  [SerializeField]
  ...
  public void UpdateIcons(Texture2D activeBranchIcon, Texture2D branchIcon,
                          Texture2D folderIcon, Texture2D globeIcon) {
    var needsLoad = ActiveBranchIcon == null || BranchIcon == null ||
                    FolderIcon == null || GlobeIcon == null;
    if (needsLoad) {
  ...
  protected Texture GetNodeIcon(TreeNode node) {
    ...
    if (node.IsActive) {
      ...
      nodeIcon = IsRemote && node.Level == 1 ? GlobeIcon : FolderIcon;
    } else {
      nodeIcon = BranchIcon;
    }
    return nodeIcon;
  ...
  protected override TreeNode CreateTreeNode(string path, string label,
                                             int level, bool isFolder,
                                             bool isActive, bool isHidden,
                                             bool isCollapsed, bool isChecked,
                                             GitBranchTreeData? treeData) {
  ...
  protected override IEnumerable<string> GetCollapsedFolders() {
    return folders.Where(pair => pair.Value.IsCollapsed)
        .Select(pair => pair.Key);
  }

  public override IEnumerable<string> GetCheckedFiles() {
    return checkedFileNodes
        .Where(pair => pair.Value.CheckState == CheckState.Checked)
        .Select(pair => pair.Key);
  }
...
public enum TreeNodeRenderResult { None, VisibilityChange, CheckChange }
}
