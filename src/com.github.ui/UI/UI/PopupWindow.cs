...
public class PopupWindow : BaseWindow {
  ...
  [SerializeField]
  private bool shouldCloseOnFinish;

  public event Action<bool> OnClose;

  public static PopupWindow OpenWindow(PopupViewType popupViewType,
  ...
  private void Open(PopupViewType popupViewType, Action<bool> onClose) {
    ...
    if (viewNeedsAuthentication) {
      ...
      foreach (var keychainConnection in Platform.Keychain.Connections
        ...
        var apiClient =
            new ApiClient(Platform.Keychain, Platform.ProcessManager,
                          TaskManager, Environment, keychainConnection.Host);
        ...
        try {
          ...
          Logger.Trace(ex, "Exception validating host {0}",
                       keychainConnection.Host);
        }
      }
