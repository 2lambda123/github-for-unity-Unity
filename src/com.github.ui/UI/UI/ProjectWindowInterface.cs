...
class ProjectWindowInterface : AssetPostprocessor {
  ...
  private const string AssetsMenuReleaseLockForced =
      "Assets/Release Lock (forced)";
  ...
  private static ILogging Logger {
    get {
      return logger = logger ?? LogHelper.GetLogger<ProjectWindowInterface>();
    }
  }
  ...
  private static IRepository Repository {
    get { return manager != null ? manager.Environment.Repository : null; }
  }
  private static IPlatform Platform {
    get { return manager != null ? manager.Platform : null; }
  }
  ...
  private static void ValidateCachedData() {
    Repository.CheckAndRaiseEventsIfCacheNewer(
        CacheType.GitStatus, lastRepositoryStatusChangedEvent);
  ...
  private static void
  RepositoryOnLocksChanged(CacheUpdateEvent cacheUpdateEvent) {
  ...
  private static void ContextMenu_Lock() {
    RunLockUnlock(IsObjectUnlocked, CreateLockObjectTask,
                  Localization.RequestLockActionTitle,
                  "Failed to lock: no permissions");
  }

  [MenuItem(AssetsMenuReleaseLock, false, 10001)]
  private static void ContextMenu_Unlock() {
    RunLockUnlock(IsObjectLocked, x => CreateUnlockObjectTask(x, false),
                  Localization.ReleaseLockActionTitle,
                  "Failed to unlock: no permissions");
  }

  [MenuItem(AssetsMenuReleaseLockForced, false, 10002)]
  private static void ContextMenu_UnlockForce() {
    RunLockUnlock(IsObjectLocked, x => CreateUnlockObjectTask(x, true),
                  Localization.ReleaseLockActionTitle,
                  "Failed to unlock: no permissions");
  }

  private static void RunLockUnlock(Func<Object, bool> selector,
                                    Func<Object, ITask> creator, string title,
                                    string errorMessage) {
    isBusy = true;
  ...
  private static bool IsObjectUnlocked(Object selected) {
    ...
    NPath assetPath =
        AssetDatabase.GetAssetPath(selected.GetInstanceID()).ToNPath();
    ...
    if (entries != null) {
      status = entries.FirstOrDefault(x => repositoryPath == x.Path.ToNPath())
                   .Status;
    }
  ...
  private static bool IsObjectLocked(Object selected,
    ...
    return locks.Any(
        x => repositoryPath == x.Path &&
             (!isLockedByCurrentUser || x.Owner.Name == currentUsername));
  }
  ...
  private static ITask CreateLockObjectTask(Object selected) {
    ...
    task.OnEnd += (_, s, ___) => {
      if (s)
        manager.TaskManager.Run(
            manager.UsageTracker.IncrementUnityProjectViewContextLfsLock, null);
    };
    return task;
  }
  ...
  private static void OnLocksUpdate() {
    ...
    // so that we can show the "this thing is locked by X" and that the
    // IsOpenForEdit call happens and the inspector is disabled. There's no way
    // to refresh the editor directly (well, there is, but it's an internal
    // api), so this just causes Unity to repaint everything. Nail, meet
    // bazooka, unfortunately, but that's the only way to do it with public APIs
    // ¯_(ツ)_/¯
  ...
  private static void OnProjectWindowItemGUI(string guid, Rect itemRect) {
    ...
    if (texture == null) {
      var path =
          gitStatusEntry.HasValue ? gitStatusEntry.Value.Path : string.Empty;
      Logger.Warning(
          "Unable to retrieve texture for Guid:{0} EntryPath:{1} Status: {2} IsLocked:{3}",
          guid, path, status.ToString(), isLocked);
      return;
    }
    ...
    if (itemRect.width > itemRect.height) {
      rect = new Rect(
          itemRect.xMax - texture.width, itemRect.y, texture.width,
          Mathf.Min(texture.height, EditorGUIUtility.singleLineHeight));
    }
    // Corner placement
    // TODO: Magic numbers that need reviewing. Make sure this works properly
    // with long filenames and wordwrap.
    else {
      ...
      var offset = new Vector2(itemRect.width * Mathf.Min(.4f * scale, .2f),
                               itemRect.height * Mathf.Min(.2f * scale, .2f));
      rect =
          new Rect(itemRect.center.x - size.x * .5f + offset.x,
                   itemRect.center.y - size.y * .5f + offset.y, size.x, size.y);
    }
