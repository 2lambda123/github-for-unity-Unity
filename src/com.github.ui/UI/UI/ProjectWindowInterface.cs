...
class ProjectWindowInterface : AssetPostprocessor
{
    ...
    private static IRepository Repository {
        get {
            return manager != null ? manager.Environment.Repository : null;
        }
    }
    private static IPlatform Platform {
        get {
            return manager != null ? manager.Platform : null;
        }
    }
    ...
    private static ITask CreateUnlockObjectTask(Object selected, bool force)
    {
        ...
        task.OnEnd += (_, s, __) => {
            if (s) manager.TaskManager.Run(manager.UsageTracker.IncrementUnityProjectViewContextLfsUnlock, null);
        };
        return task;
    }
    ...
    private static ITask CreateLockObjectTask(Object selected)
    {
        ...
        task.OnEnd += (_, s, ___) => {
            if (s) manager.TaskManager.Run(manager.UsageTracker.IncrementUnityProjectViewContextLfsLock, null);
        };
        return task;
    }
