...
class GitHubEnterpriseAuthenticationView : Subview {
  ...
  private const string CredentialsNeedRefreshMessage =
  ...
  private const string NeedAuthenticationMessage =
      "We need you to authenticate first";
  ...
  private const string TwofaDescription =
      "Open the two-factor authentication app on your device to view your 2FA code and verify your identity.";
  ...
  [SerializeField]
  private string two2fa = string.Empty;
  [SerializeField]
  ...
  [SerializeField]
  private bool hasServerMeta;
  [SerializeField]
  private bool verifiablePasswordAuthentication;
      ...
      GUILayout.BeginVertical();
      {
        if (!hasServerMeta) {
          ...
          EditorGUI.BeginDisabledGroup(true);
          {
            ...
            {
              serverAddress = EditorGUILayout.TextField(
                  ServerAddressLabel, serverAddress, Styles.TextFieldStyle);
            }
            GUILayout.EndHorizontal();
          }
          ...
          if (!need2fa) {
            ...
            if (OAuthCallbackManager.IsRunning) {
              ...
              GUILayout.BeginHorizontal();
              {
                ...
                if (GUILayout.Button("Sign in with your browser",
  ...
  private void HandleEnterPressed() {
    ...
    enterPressed = Event.current.keyCode == KeyCode.Return ||
  ...
  private void OnGUITokenLogin() {
    EditorGUI.BeginDisabledGroup(isBusy);
    {
      ...
      {
        token =
            EditorGUILayout.TextField(TokenLabel, token, Styles.TextFieldStyle);
      }
      GUILayout.EndHorizontal();
  ...
  private void OnGUI2FA() {
    GUILayout.BeginVertical();
    {
      ...
      {
        EditorGUILayout.Space();
        two2fa = EditorGUILayout.TextField(TwofaLabel, two2fa,
                                           Styles.TextFieldStyle);
  ...
  private void OnOAuthCallback(string state, string code) {
    TaskManager.RunInUI(() => {
      if (state.Equals(oAuthState)) {
        isBusy = true;
        authenticationService.LoginWithOAuthCode(
            code, (b, s) => TaskManager.RunInUI(() => DoOAuthCodeResult(b, s)));
      }
    });
  }
  ...
  private void DoServerMetaResult(GitHubHostMeta gitHubHostMeta) {
    ...
    verifiablePasswordAuthentication =
        gitHubHostMeta.VerifiablePasswordAuthentication;
    isBusy = false;
    Redraw();
  }
  ...
  private AuthenticationService GetAuthenticationService(string host) {
    if (authenticationService == null ||
        authenticationService.HostAddress.WebUri.Host != host) {
      authenticationService = new AuthenticationService(
          host, Platform.Keychain, Manager.ProcessManager, Manager.TaskManager,
          Environment);
