...
function ApiWrapper(host) {
  ...
  this.octokit.authenticate({type : "oauth", token : config.token});
}
    ...
    that.octokit.users.getOrgs(position, function(error, result) {
      if (error) {
        ...
        if (result.data.length == perPageCount) {
          ...
          var organizationLogins = organizations.map(function(
              item) { return {name : item.name || "", login : item.login}; });
...
ApiWrapper.prototype.publish = function(name, desc, private, organization,
                                        callback) {
  var callbackHandler = function(error, result) {
    callback(error,
             (!result) ? null : [ result.data.name, result.data.clone_url ]);
  };
  ...
  if (organization) {
    ...
    this.octokit.repos.create(
        {name : name, description : desc, private : private}, callbackHandler);
  }
};
